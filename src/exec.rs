...
mod tests {
    ...
    fn how_many_default_bindings_are_live() {
        ...
        // `cmd-set-text`, `command-accept` and the readline bindings, 106 with hints, 108 once
        // `tab-pin` and `tab-mute` had a flag and a strip to show it on. Raise this when a
        // milestone raises the number, never to make a failing build pass.
        assert_eq!(live, 108, "the live-binding count moved");
    }
