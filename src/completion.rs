...
pub trait Sources: Send + Sync {
    ...
    /// Walks `CompletionHistory` in **`last_atime` descending order** — that ordering is the
    /// contract, and the index on `last_atime` is what makes it free. Stops early when `visit`
    /// returns [`Flow::Stop`].
    fn history(&self, visit: &mut dyn FnMut(HistoryRow<'_>) -> Flow);
}
...
mod tests {
    ...
    fn ranges_are_utf16_units_not_bytes() {
        ...
        impl Sources for One {
            ...
            fn history(&self, _visit: &mut dyn FnMut(HistoryRow<'_>) -> Flow) {}
        }
