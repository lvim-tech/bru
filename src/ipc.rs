...
/// Until tabs exist there is one page, so the strip shows one entry built from the same state the
/// bar uses. The tab list itself arrives with M5 and replaces this.
fn tabs_json() -> String {
    let Ok(bar) = bar().lock() else {
        return "{\"tabs\":[]}".to_string();
    };
    if bar.url.is_empty() && bar.title.is_empty() {
        return "{\"tabs\":[]}".to_string();
    }
    format!(
